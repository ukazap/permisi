  ...
  module PermissionUtil
    class << self
      ...
      def transform_namespace(namespace, current_path: nil)
        transformed = ::HashWithIndifferentAccess.new

        namespace.each_pair do |key, value|
          raise InvalidNamespace, "`#{[current_path, key].compact.join(".")}` should be an array" unless value.is_a? Array
          ...
          value.each.with_index do |arr_v, arr_i|
            if arr_v.is_a?(Symbol)
              ...
              transform_namespace(arr_v, current_path: [current_path, key].compact.join(".")).each_pair do |ts_k, ts_v|
                transformed[key] ||= ::HashWithIndifferentAccess.new
              ...
              raise InvalidNamespace, "`#{[current_path, key].compact.join(".")}[#{arr_i}]` should be a symbol or a hash"
            end
          end
        end
      ...
      def __deeply_sanitize_permissions(permission_hash, template: {})
        sanitized = HashWithIndifferentAccess.new
